...
axios.get("http://localhost:3000/api/orders/999").then(({ result }) => { // requesting an order that does not exist (/999) to se how to "Handling Errors with Promises"
  if (result.status === 200) { // HTTP status codes, a 200 is a success
    showOrderList("#order-list", result.data); // if it's success, we'll show the order list
  } else {
    showError("Error!"); // anything that's not a 200 is an error so, if it fails, we'll show an error.
  }
});
...
With your console openned as you browse to localhost:3000/myOrders You'll notice that our orders list is empty. The screen just has the Orders header and nothing else. And in your console, you have two errors: 
 ...
 What does uncaught in promise mean? Remember that a promise can have three states. The promise is in a pending state until the API returns something, middle state, the fulfilled state. But this time, the promise didn't succeed. So now we have to deal with a rejected state.
